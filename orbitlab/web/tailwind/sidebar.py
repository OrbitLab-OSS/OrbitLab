...
from orbitlab.web.tailwind import Buttons, FieldSet, Select
from orbitlab.web.tailwind.dialog import Dialog
from orbitlab.web.tailwind.logo import OrbitLabLogo
from orbitlab.web.tailwind.menu import Menu
...
class ClusterSettingsDialog(EventGroup):
    ...
    async def load(state: ClusterSettingsDialogState) -> FrontendEvents:
        ...
        state.default_node = defaults.node
        state.default_vztmpl = defaults.vztmpl
        state.default_snippets = defaults.snippets
        state.default_imports = defaults.imports
        state.default_iso = defaults.iso
        state.default_backup = defaults.backup
        state.default_rootdir = defaults.rootdir
        state.default_images = defaults.images
        return Dialog.open(ClusterSettingsDialog.dialog_id)

    @staticmethod
    @rx.event
    async def set_default_node(state: ClusterSettingsDialogState, node: str) -> None:
        state.default_node = node
        state.default_vztmpl = ""
        state.default_imports = ""
        state.default_rootdir = ""
    ...
    async def save(state: ClusterSettingsDialogState) -> FrontendEvents:
        ...
        from orbitlab.redis.models import Defaults
        
        await ClusterClient().set_defaults(
            defaults=Defaults(
                node=state.default_node,
                vztmpl=state.default_vztmpl,
                imports=state.default_imports,
                rootdir=state.default_rootdir,
                snippets=state.default_snippets,
                iso=state.default_iso,
                backup=state.default_backup,
    ...
    def __new__(cls) -> rx.Component:
        ...
        storage_options = SelectOptions.node_storage_options.get(
            ClusterSettingsDialogState.default_node, default={},
