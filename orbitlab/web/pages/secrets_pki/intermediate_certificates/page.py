...
@rx.event
async def set_filter(state: IntermediateCAState, cert_filter: Literal["All", "Valid", "Warning", "Expired"]):
...
class IntermediateCertificateAuthorities:
    ...
    def __filter_button__(cls, label: str) -> rx.Component:
        return rx.el.button(
            label,
            on_click=lambda: set_filter(label),
    ...
    def __new__(cls):
        return rx.el.div(
            ...
            GridList(
                rx.foreach(
                    IntermediateCAState.filtered_certificates,
