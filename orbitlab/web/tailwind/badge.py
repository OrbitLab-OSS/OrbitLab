...
class WithStatus:
    ...
    color_classes: ClassVar[dict[Colors, str]] = {
        "default": (
            ...
            "dark:from-[#0E1015] dark:to-[#181B22] dark:text-gray-300"
        ),
        "blue": (
            "bg-gradient-to-b from-[#E0F2FE] to-[#BFDBFE] text-[#1E63E9] "
            "dark:from-[#1E63E9] dark:to-[#36E2F4] dark:text-[#36E2F4]"
        ),
        "green": (
            "bg-gradient-to-b from-[#16A34A] to-[#4ADE80] text-[#16A34A] "
            "dark:from-[#16A34A] dark:to-[#4ADE80] dark:text-[#4ADE80]"
        ),
        "red": (
            "bg-gradient-to-b from-[#FEE2E2] to-[#FCA5A5] text-[#DC2626] "
            "dark:from-[#DC2626] dark:to-[#F87171] dark:text-[#F87171]"
        ),
        "orange": (
            "bg-gradient-to-b from-[#FFEDD5] to-[#FED7AA] text-[#EA580C] "
            "dark:from-[#EA580C] dark:to-[#FB923C] dark:text-[#FB923C]"
        ),
    }
    ...
    def __new__(
        ...
        *,
        ...
        if status_content is None:
            return rx.el.span(
                ...
                rx.el.span(
                    rx.el.span(
                        class_name=(
                            "absolute top-0 right-0 rounded-full size-3 animate-ping data-[animate=false]:hidden "
                            f"{cls.color_classes.get(color, cls.color_classes['default'])}"
                        ),
                        data_animate=animate
                    ),
                    class_name=(
                        "absolute -top-1 -right-1 rounded-full size-3 data-[disabled=true]:hidden "
                        f"{cls.color_classes.get(color, cls.color_classes['default'])}"
                    ),
        ...
        return rx.el.span(
            component,
            rx.el.span(
                rx.text(status_content, class_name="text-xs px-1 text-white dark:text-black"),
                rx.el.span(
                    class_name=(
                        "absolute -top-1 -right-1 h-full w-full rounded-full animate-ping data-[animate=false]:hidden "
                        f"{cls.color_classes.get(color, cls.color_classes['default'])}"
                    ),
                    data_animate=animate
                ),
                class_name=(
                    "absolute -top-1 -right-1 rounded-full h-fit w-fit data-[disabled=true]:hidden "
                    f"{cls.color_classes.get(color, cls.color_classes['default'])}"
                ),
                data_disabled=disabled,
            ),
