...
class DownloadApplianceState(rx.State):
    ...
    async def load(self) -> None:
        ...
        existing = [apl.spec.template for apl in await self.get_var_value(OrbitLabState.base_appliances)]
        for appliance in await ProxmoxComputeTemplates().list_appliances():
...
class CustomApplianceState(rx.State):
    ...
    async def load_appliance(self, appliance_id: str) -> None:
        ...
        self.form_data = {
            ...
            "base_appliance": appliance.config.base_appliance_id,
            "node": appliance.config.node,
            "storage": appliance.config.storage,
            "rootfs": appliance.config.storage,
            "sector": appliance.config.sector,
            "swap": appliance.config.swap,
            "memory": appliance.config.memory,
        }
